...
// ---------- Nav + Search helpers --------------------------------------------
const hrefFor = (slug: Slug) => `/topics/mobile-security/${slug}`;
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      try {
        ...
        console.assert(hrefFor("intro") === "/topics/mobile-security/intro", "hrefFor(intro)");
        console.assert(hrefFor("oauth") === "/topics/mobile-security/oauth", "hrefFor(oauth)");
    ...
    <div className="min-h-screen grid grid-cols-[280px_1fr] bg-slate-950 text-slate-100">
      ...
      <aside className="sticky top-0 h-screen overflow-auto border-r border-slate-800/70 bg-slate-900/60">
        <div className="flex items-center gap-2 px-4 py-4 border-b border-slate-800/70">
          ...
          <h1 className="text-sm font-bold tracking-wide">Iron‑Codex • Mobile Security</h1>
        </div>
