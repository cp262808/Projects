...
// ---------- Nav + Search helpers --------------------------------------------
const hrefFor = (slug: Slug) => `/topics/secrets-management/${slug}`;
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      try {
        ...
        console.assert(hrefFor("intro") === "/topics/secrets-management/intro", "hrefFor(intro)");
        console.assert(hrefFor("oauth") === "/topics/secrets-management/oauth", "hrefFor(oauth)");
    ...
    <div className="min-h-screen grid grid-cols-[280px_1fr] bg-slate-950 text-slate-100">
      ...
      <aside className="sticky top-0 h-screen overflow-auto border-r border-slate-800/70 bg-slate-900/60">
        <div className="flex items-center gap-2 px-4 py-4 border-b border-slate-800/70">
          ...
          <h1 className="text-sm font-bold tracking-wide">Iron‑Codex • Secrets Management</h1>
        </div>
      ...
      <main className="px-5 sm:px-8 py-6 max-w-[1100px]">
        <div className="text-slate-400 text-sm mb-2">Cybersecurity › Secrets Management</div>
